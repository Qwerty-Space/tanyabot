...
async def currency(event):
    ...
    result = round(c.convert(event.pattern_match.group(1), event.pattern_match.group(2), event.pattern_match.group(3)), 2)
    print(f"[{event.date.strftime('%c')}] [{sender.id}] {sender.username}: {event.pattern_match.string}: {result}")
    await event.reply(f"**{event.pattern_match.group(1)} {event.pattern_match.group(2)} is:**  `{result} {event.pattern_match.group(3)}`")
